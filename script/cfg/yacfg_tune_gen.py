...
class CopyKeycloakDependencies(apply_security.ExtraResource):
    def create(self, dest_dir):
        ...
        if yacfg_profile_name == 'artemis':
            ...
            # remove this hack >>>>>>
            # for dev as I'm behind GFW.
            klib_dir = "/opt/amq-broker/script/cfg/keycloak"
            for item in os.listdir(klib_dir):
                s = os.path.join(klib_dir, item)
                d = os.path.join(dest_dir.joinpath("lib").absolute(), item)
                shutil.copy2(s, d)
            # remove this hack <<<<<<
...
if __name__ == '__main__':
    ...
    print("!!!! Now using hard coded profile 2.17.0, real version", yacfg_profile_version)
    yacfg_profile_version = '2.17.0'
